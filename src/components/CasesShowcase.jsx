...
export default function CasesShowcase() {
  ...
  return (
    <Section id="cases" className="py-20 md:py-28">
      ...
      />
      {/* mobile carousel */}
      <div className="mt-14 sm:hidden flex gap-4 overflow-x-auto snap-x snap-mandatory pb-4 -mx-5 px-5 scrollbar-hide">
        {cards.map((_, i) => (
          <div key={i} className="snap-start shrink-0 w-[75vw] group relative flex aspect-[4/5] flex-col justify-between overflow-hidden rounded-xl3 bg-white/45 p-6 ring-1 ring-dashed ring-ink/[0.12] backdrop-blur">
      ...
      {/* desktop grid */}
      <div className="hidden sm:grid mt-14 gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {cards.map((_, i) => (
